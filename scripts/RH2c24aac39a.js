(function () {
    var O = function () {
        ...
        RH.trackReferral = function (a, c, d) {
            a && 0 < a.trim().length ? g.tools.ajax.request(g.tools.getWidgetUrl(config[Object.keys(config)[0]]) + "/find_list_and_settings", "POST", {
                ui: a,
                user_uuid: b.uuid
            }, function (f) {
                if(f.response == "ok"){
                    f.list_uuid.forEach(function(list_uuid) {
                        ...
                        config[list_uuid] != undefined && g.methods.trackReferral(a, c, d, config[list_uuid], window[rh_variable]);
                    })
                }else{
        ...
        RH.organicTrackReferral = function (data, c) {
            var list_uuid = null;
            let rh_variable = null;
            g.tools.ajax.request(g.tools.getWidgetUrl(config[Object.keys(config)[0]]) + "/find_list_and_settings", "POST", {
                ui: data,
                user_uuid: b.uuid
            }, function (f) {
                if(f.response == "ok"){
        ...
        g.methods = {
            identify: function (a, c, d, config, RH) {
                ...
                if (config.settings.recaptcha.enable) r.session = !0, console.warn("[ReferralHero] Identify() disabled because Recaptcha is enabled."); else if (a) if (ui && "" != ui.trim()) {
                    ...
                    }, g.tools.ajax.request(g.tools.getWidgetUrl(config) + "/post", "POST", c, function (e) {
            ...
            trackReferral: function (a, c, d, config, RH){
                ...
                config.settings.track_events ? a && 0 < a.trim().length ? (a = {
                    ...
                    typeof c && (a.name = c.name, a.conversion_value = c.value, a.transaction_id = c.transaction_id, a.conversion_category = c.category, a.product_id = c.product_id), g.tools.ajax.request(g.tools.getWidgetUrl(config) + "/track_referral", "POST", a, function (e) {
            ...
            organicTrackReferral: function (a, c, config, RH) {
                ...
                (config.settings.track_events || config.settings.allow_organic_traffic_tracking) ? ui && 0 < ui.trim().length ? (a = {
                        uuid: config.uuid,
                        url: window.location.href,
                        fingerprint: RH.fingerprint,
                        source: RH.source,
                    ...
                    }, g.tools.ajax.request(g.tools.getWidgetUrl(config) + "/organic_track_referral", "POST", a, function (e) {
            ...
            pendingReferral: function (a, c, config, RH) {
                ...
                config.settings.track_events ? ui && 0 < ui.trim().length ? (a = {
                        ...
                        landing_page_url: window.location.href,
                        tags: a.tags
                    }, g.tools.ajax.request(g.tools.getWidgetUrl(config) + "/pending_referral", "POST", a, function (e) {
            ...
            trackTransaction: function (a, config, RH) {
                ...
                a ? g.tools.ajax.request(g.tools.getWidgetUrl(config) + "/track_transaction", "POST", {
                    uuid: config.uuid,
                    amount: a.amount,
                    transaction_id: a.transaction_id,
        ...
        g.loadFingerPrint = function (RH) {
            "undefined" !== typeof Fingerprint2 ? (new Fingerprint2).get(function (a, c) {
                RH.fingerprint = a;
                r.fingerprinting = !0
            }) : 10 > L ? (L++, setTimeout(g.loadFingerPrint(RH), 50)) : r.fingerprinting = !0
        };
        ...
        g.libraries = {
            fp: function (RH) {
                if (w) r.fingerprinting = !0; else if ("undefined" ===
                    typeof window.Fingerprint2) {
                        if(!global_config["enableFP"]){
							global_config["enableFP"] = true
							var a = g.add_script("https://cdnjs.cloudflare.com/ajax/libs/fingerprintjs2/1.6.1/fingerprint2.min.js")
            ...
            recaptcha: function (config, RH) {
                ...
                } else g.loadReCaptcha(config, RH)
            }
        };
        ...
        g.tools = {
            getOriginalHost: function () {
                ...
                if (c && (c == this.getParams("mwr") || (c != this.getParams("mwr") && anonymous_visitor == this.getParams("mwr")) || (this.getParams("mwr")) == undefined)) RH.referrer = c, RH.referrer_name = rnc, r.referrer = !0; else {
                                ...
                                if(!config.settings.allow_organic_traffic_tracking){  
                                    ...
                                    if (RH.referrer_name) {
                                        ...
                                        elements.forEach(function(element) {
                                            element.innerHTML = element.innerHTML.replace(/<span class="mtr-hide">(%referrer_name%)<\/span>/g, RH.referrer_name || '').replace(/<span class="mtr-hide">(%advocate_first_name%)<\/span>/g, RH.advocate_first_name || '').replace(/<span class="mtr-hide">(%advocate_last_name%)<\/span>/g, RH.advocate_last_name || '');
                                        });
                                    }
                                }else{
									RH.referrer = d;
               ...
               RH.anonymous = c ? c : '';
                ...
                //    if (c) RH.anonymous = c, r.anonymous = !0; else {
                        var d = ref || g.tools.getParams("mwr");
                ...
                setTimeout( function(){
                    g.tools.ajax.request(g.tools.getWidgetUrl(config) + "/check_nps_visit", "POST", {
                        uuid: config.uuid,
                        host: config.defaults.default_url,
                        landing_page_url: window.location.href,
                        visit_page: window.location.href,
                ...
                if (true) {
                    if (!r.session_cookie) {
                        ...
                        c && this.ajax.request(this.getWidgetUrl(config) + "/check_subscriber", "POST", {
                ...
                if (config.settings.enable_visit_url && window.location.href.split('?')[0] == config.settings.track_visit_url) {
                    ...
                    g.tools.ajax.request(g.tools.getWidgetUrl(config) + "/track_referral", "POST", a, function (e) {
                ...
                0 < a.length ? g.identify(a[0][1], a[0][2], null, config, RH) : r.session = !0
            }, getParams: function (a) {
                var c = {};
                window.location.search.replace(/[?&]+([^=&]+)=([^&]*)/gi, function (d, f, e) {
                    c[decodeURIComponent(f)] = decodeURIComponent(e)
                });
                return a ? c[a] : c
                ...
                e = c.querySelector("[name='extra_field_2']"), eee = c.querySelector("[name='extra_field_3']"), eeee = c.querySelector("[name='extra_field_4']"), cc = c.querySelector("[name='country_code']"), o= c.querySelector("[name='phone_number']"), l = c.querySelector("[name='crypto_wallet_address']"), z = c.querySelector("[name='crypto_wallet_provider']"), s = c.querySelector("[name='other_identifier']"), x = c.querySelector("[name='option_field']") ;
                ...
                {"country":"Netherlands","code":"31","iso":"NL","format":"8"},
                ...
                {"country":"Nigeria","code":"234","iso":"NG","format":"8"},
                ...
                this.ajax.request(config.settings.lambda.email_endpoint, "POST", {
                ...
                this.ajax.request(config.settings.lambda.sms_endpoint,
                    "POST", {
                ...
                for(var attribute in data){
                    ...
                    if(data["id"] != undefined && data["id"].startsWith("button-link") && attribute == "href" && RH.optin_data){
                        el.setAttribute(attribute, data[attribute]+ "?rh_sub_id=" + RH.optin_data.id)
                    }else{
            ...
            }, designElement: function (designer_data, config, RH){
                var element_json = designer_data.element_attributes.replace("justify-center", "mtr-justify-center").replace("items-center", "mtr-items-center").replace(" mb-4", " mtr-mb-4").replace("flex", "mtr-flex");
            ...
            check_discord_join: function(invite_id, sub_id, interval, checked_background, checked, the_social_bg_color, RH){
                RH.optin_data && g.tools.ajax.request(g.tools.getWidgetUrl(config) + "/check_discord_join", "POST", {
                ...
                RH.optin_data && g.tools.ajax.request(g.tools.getWidgetUrl(config) + "/check_telegram_join", "POST", {
                    subscriber_id: RH.optin_data.id,
                    invite_id: invite_id
                }, function (z) {
                    if (z.response == "success"){
                ...
                if(!c){
                    ...
                    d && this.ajax.request(this.getWidgetUrl(config) + "/find_subscriber_in_list", "POST", {
                        code: d
                    }, function (f) {
                        if (f.response == "ok"){
                ...
                return text.replace(phoneRegex, function(match, phone) {
                    ...
                    if (cleanNumber.length === 10) {
                        return ` <a href="tel:${cleanNumber}" style="color: inherit;">${phone}</a>`; // Maintain spacing for readability
                    } else {
                        return match;
                    }
                });
        ...
        g.callbacks = {
            success: function (a, config, RH) {
                ...
                "subscriber_created" != a.response || w || E || (false  ? g.tools.sendSMS(a.data.phone_number, a.confirmation_links.sms, null, null, config) : (false) && g.tools.sendEMail(a.data.email, a.data.name, a.confirmation_links.email, null, null, config))) : (mtid("mtr-form-submit-button") && (mtid("mtr-form-submit-button").disabled = !1, mtid("mtr-form-submit-button").innerHTML = q ? config.settings.form.submit_button.check_position : config.settings.form.submit_button.text), "server_problem" == a.response ?
                ...
                config.callbacks.hasOwnProperty("afterSuccess") && config.callbacks.afterSuccess(a)
                 ...
                 ((mtid(config.defaults.form_container_id) || mtid(config.defaults.form_container_id_two) || mtid(config.defaults.sharing_form_container_id))? (config.settings.sharing.redirection.enable ? window.location.replace(encodeURI(config.settings.sharing.redirection.url +
                    "?rh_email=" + RH.optin_data.email + "&rh_name=" + RH.optin_data.name + "&rh_extra_field=" + RH.optin_data.extra_field + "&rh_extra_field_2=" + RH.optin_data.extra_field_2 + "&rh_extra_field_3=" + RH.optin_data.extra_field_3 + "&rh_extra_field_4=" + RH.optin_data.extra_field_4 + "&rh_code=" + RH.optin_data.code + "&rh_sub_id=" + RH.optin_data.id)) : g.sharing.open(config, RH)) : ( mtid(config.defaults.signup_form_container_id) && config.settings.signup_widget_form.redirection.enable ) ? window.location.replace(encodeURI(config.settings.signup_widget_form.redirection.url +
                    "?rh_email=" + RH.optin_data.email + "&rh_name=" + RH.optin_data.name + "&rh_extra_field=" + RH.optin_data.extra_field + "&rh_extra_field_2=" + RH.optin_data.extra_field_2 + "&rh_extra_field_3=" + RH.optin_data.extra_field_3 + "&rh_extra_field_4=" + RH.optin_data.extra_field_4 + "&rh_code=" + RH.optin_data.code + "&rh_sub_id=" + RH.optin_data.id)) : (( q || (mtid("maitre-floating-button") && !(mtid(config.defaults.signup_form_container_id)))  || mtid(config.defaults.inline_button_container_id)) ? g.sharing.open(config, RH) : g.callbacks.post_success_msg(a, config, RH)))
            }, trackEvents: function () {
                var a = b.queue.filter(function (d) {
                    return "track" == d[0]
                });
                if (0 < a.length) for (var c = 0; c < a.length; c++) g.trackEvent(a[c][1], a[c][2])
                ...
                if(config.settings.enable_features.signup_widget && config.settings.signup_widget_form.confirmation_screen_popup.enable && mtid(config.defaults.signup_form_container_id) && !config.settings.sharing.popup){
                    g.generate.popup(g.generate.thankyou_screen(config), config)
                }
                        ...
                        if (config.settings.signup_widget_form.thankyou_screen_settings){
                            ...
                            for( var i in designer_data){
                                ...
                                if (el.id == "thankyou-form"){
                                    ...
                                    t_el.style.backgroundSize = t_el.getAttribute("data-image-size");
                                }else{
                                    if(el.classList.contains('mtr-coupon-field')){
                                        if(ap_el){
        ...
        g.form = {
            incomplete: function (a, config, RH, qf = !1, form = config.settings.form) {
                ...
                if (qf){
                    return config.settings.quick_add_form.email.require && config.settings.quick_add_form.email.required && "" == c.value.trim() || config.settings.quick_add_form.name.require && config.settings.quick_add_form.name.required && "" == d.value.trim() && !q || config.settings.quick_add_form.extra_field.require && config.settings.quick_add_form.extra_field.required && "" == f.value.trim() && !q || config.settings.quick_add_form.extra_field_2.require && config.settings.quick_add_form.extra_field_2.required && "" == e.value.trim() && !q || config.settings.quick_add_form.extra_field_3.require && config.settings.quick_add_form.extra_field_3.required && "" == eee.value.trim() && !q || config.settings.quick_add_form.extra_field_4.require && config.settings.quick_add_form.extra_field_4.required && "" == eeee.value.trim() && !q || config.settings.quick_add_form.option_field.require && config.settings.quick_add_form.option_field.required && "" == x.value.trim() && !q ? "form_incomplete" : !config.settings.quick_add_form.terms_conditions.require || a.checked || q ? "valid" : "terms_not_accepted"
                }
                else{
                    return form.email.require && form.email.required && "" == c.value.trim() && !q || form.crypto_wallet_address.require && form.crypto_wallet_address.required && "" == l.value.trim() && !q || form.crypto_wallet_address.require && config.settings.sharing.verification.crypto_wallet_confirmation && "" == z.value.trim() && !q || form.other_identifier.require && "" == s.value.trim() && !q || form.name.require && form.name.required && "" == d.value.trim() && !q || form.extra_field.require && form.extra_field.required && "" == f.value.trim() && !q || form.extra_field_2.require && form.extra_field_2.required && "" == e.value.trim() && !q || form.extra_field_3.require && form.extra_field_3.required && "" == eee.value.trim() && !q || form.extra_field_4.require && form.extra_field_4.required && "" == eeee.value.trim() && !q || form.option_field.require && form.option_field.required && "" == x.value.trim() && !q || form.phone_number.require && form.phone_number.required && "" == o.value.trim() && !q || form.crypto_wallet_address.require && form.crypto_wallet_address.required && "" == l.value.trim() && !q ? "form_incomplete" : !form.terms_conditions.require || a.checked || q ? "valid" : "terms_not_accepted"
                }
                
            }, submit: function (a, config, RH, qf = !1, form = config.settings.form, widget_type = null) {
                RH_JM = 1;
                if(mtid("mtr-form-submit-button")){
                    mtid("mtr-form-submit-button").innerHTML = g.tools.tailwindSpinner();
                    mtid("mtr-form-submit-button").disabled = !0
                ...
                var c = {
                    ...
                    uuid: config.uuid,
                    host: config.defaults.default_url,
                    referrer: !qf ? (RH.referrer || g.tools.referrerCookie("__maitre-referrer-MF")) : RH.optin_data.code,
                    status: !qf ? (( config.settings.track_events && ( RH.referrer || RH.anonymous || g.tools.referrerCookie("__maitre-referrer-MF") )) ? "custom_event_pending" : "registered") : (config.settings.track_events ? "custom_event_pending" : "registered"),
                ...
                if (!qf) {
                    ...
                    c.sub_id = RH.anonymous ||  g.tools.readCookie(c_name);
                ...
                if (qf){
                    g.tools.asyncRequest.request(g.tools.getWidgetUrl(config) + "/post", "POST", c, function (d) {
                        ...
                        "error" == d.response ? config.callbacks.hasOwnProperty("error") ? (console.error(d.message), config.callbacks.error(a)) : alert_or_console(config.settings.alerts.subscriber_not_found) : (RH.response = Maitre.response = d.response,
            ...
            }, codeVerification: function (data, config, RH) {
                var c = g.tools.readCookie("__maitre-session-" + config.uuid) || RH.optin_data.id;
        ...
        g.generate = {
            stylesheet: function (a) {
                ...
                if (config.settings.form.terms_conditions.require) {
                    ...
                    A.innerText = config.settings.form.terms_conditions.text;
                    ...
                    z.setAttribute("href", config.settings.form.terms_conditions.url);
                ...
                config.settings.form.terms_conditions.require && d.appendChild(v);
                ...
                c.addEventListener("submit", function (J) {
                    ...
                    "valid" == form_validity ? config.settings.recaptcha.enable && C ? grecaptcha.execute(F) : (data = g.tools.getFormValues(c, RH), g.form.submit(data, config, RH, !1, config.settings.form, "dashboard_widget")) : ("form_incomplete" == form_validity ? alert_or_console(config.settings.alerts.form_incomplete) : "terms_not_accepted" == form_validity ? alert_or_console(config.settings.alerts.terms_conditions) : "invalid_phone_number" == form_validity &&
                ...
                a.innerText = config.settings.floating_button.text;
                ...
                y.innerHTML = el.getAttribute('data-copy-button-text-change') || config.settings.sharing.referral_link.copy_button;
                ...
                if (el.getAttribute('data-copy-button-type') == 'text'){
                    y.style.fontFamily = g.tools.findFont(el.getAttribute('data-copy-button-font-family')) || 'inherit';
                }else if(el.getAttribute('data-copy-button-type') == 'icon'){
                    y.innerHTML = '<i style="font-family:Material Icons; font-size:20px;" class="material-icons">content_copy</i>';
                ...
                t.style.backgroundColor = "#f1f0f5";
                ...
                sa.forEach(function (p) {
                    ...
                    outer_div.classList.add("mtr-col-span-1", "mtr-bg-white", "mtr-flex", "mtr-items-center", "mtr-justify-between");
                ...
                a.addEventListener("click", function (c) {
                    c.preventDefault();
                    config.settings.sharing.open_if_signed_up && g.tools.readCookie("__maitre-session-" + config.uuid) ? ((g.tools.getSessionCookie(!0, config, RH), g.generate.popup(g.generate.thankyou_screen(config), config)) && g.generate.popup(g.generate.thankyou_screen(config), config)) : g.generate.popup(g.generate.signup_widget_form(config, RH), config)
                });
                return a
            }, inline_button: function(config, RH){
                var a = mtg("div");
                a.id = "maitre-inline-button";
                a.innerText = config.settings.inline_button.text;
                ...
                if(el.getAttribute("data-rewards-orientation")=="row" && ((new_sorted_array.length > 2 && !g.tools.mobileCheck()) || (new_sorted_array.length >= 2 && g.tools.mobileCheck()))){
                    ...
                    if(!g.tools.mobileCheck()){
                        swiper_prev.classList.add("swiper-button-prev", "swiper-prev")
                        swiper_next.classList.add("swiper-button-next", "swiper-next")
                        rewards_counter.classList.add("swiper-counter");
                    }else{
                        swiper_prev.classList.add("swiper-button-prev", "swiper-prev-mobile")
                        swiper_next.classList.add("swiper-button-next", "swiper-next-mobile")
                ...
                0 < new_sorted_array.length && (new_sorted_array.forEach(function (e, i) {
                    ...
                    t.style.color = el.getAttribute('data-reward-icons-color') || '#000000';
                    if (RH.optin_data.unlocked_rewards.includes(e.id)) {
                      t.style.backgroundColor = el.getAttribute("data-unlocked-reward-color") || "#B1E2E8";
                      t.classList.add("reward-referrals-"+id_num+"-unlocked");
                    }else{
                      t.style.backgroundColor = el.getAttribute("data-locked-reward-color") || "#EABB99";
                      t.classList.add("reward-referrals-"+id_num+"-locked");
                    }
                    ...
                    t.style.lineHeight = el.getAttribute('data-points-line-height')+'px' || "15px";
                    ...
                    reward_referrals_div.appendChild(t);
                    ...
                    reward_referrals_below_div.classList.add("reward-referrals-below-div", "reward-referrals-below-div-"+id_num, "hidden");
                    ...
                    if (RH.optin_data.unlocked_rewards.includes(e.id)){
                    progress_bar.innerHTML = `<span class="mtr-flex mtr-h-9 mtr-items-center" style="margin-top: -10px;"><span class="mtr-relative mtr-flex mtr-h-[9px] mtr-z-20 mtr-w-[9px] mtr-left-[3px] -top-3-mtr mtr-items-center mtr-justify-center mtr-rounded-full mtr-bg-c-green group-hover:mtr-bg-c-green"></span></span><div class="mtr-absolute mtr-z-20 mtr-left-4 mtr-top-0 mtr-left-[6px] mtr-h-full mtr-w-[3px] mtr-bg-c-green" aria-hidden="true"></div><div class="mtr-absolute mtr-z-10 mtr-left-4 mtr-top-0 mtr-left-[6px] mtr-h-full mtr-w-[3px] mtr-bg-white" aria-hidden="true"></div><span class="mtr-absolute mtr-bottom-0 -left-[0.5px]-mtr mtr-z-30 mtr-flex mtr-h-[17px] mtr-w-[17px] mtr-items-center mtr-justify-center mtr-rounded-full mtr-bg-c-green group-hover:mtr-bg-c-green mtr-shrink-0" style="margin-bottom:${mb_progress_bar}px;"><svg class="h-3 w-3 mtr-text-white" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path fill-rule="evenodd" d="M16.704 4.153a.75.75 0 01.143 1.052l-8 10.5a.75.75 0 01-1.127.075l-4.5-4.5a.75.75 0 011.06-1.06l3.894 3.893 7.48-9.817a.75.75 0 011.05-.143z" clip-rule="evenodd"></path></svg></span>`
                    }else{
                    progress_bar.innerHTML = `<span class="mtr-flex mtr-h-9 mtr-items-center" style="margin-top: -10px;"><span class="mtr-relative mtr-flex mtr-h-[9px] mtr-z-20 mtr-w-[9px] mtr-left-[3px] -top-3-mtr mtr-items-center mtr-justify-center mtr-rounded-full mtr-bg-c-green group-hover:mtr-bg-c-green"></span></span><div class="mtr-absolute mtr-z-10 mtr-left-4 mtr-top-0 mtr-left-[6px] mtr-h-full mtr-w-[3px] mtr-bg-white" aria-hidden="true"></div><span class="mtr-absolute mtr-bottom-0 -left-[0.5px]-mtr mtr-z-30 mtr-flex mtr-h-[17px] mtr-w-[17px] mtr-items-center mtr-justify-center mtr-rounded-full mtr-bg-white mtr-border-2 mtr-border-c-gray-200 mtr-shrink-0" style="margin-bottom:${mb_progress_bar}px;"></span>`
                    }
                    ...
                    if(el.getAttribute("data-rewards-orientation")=="column"){
                      ...
                      if(label_present){
                        reward_referrals_below_div.style.minWidth = "70px";
                ...
                box.style.fontSize = el.getAttribute('data-coupon-font-size')+'px' || "14px";
                box.style.lineHeight = el.getAttribute('data-coupon-line-height')+'px' || "15px";
                ...
                sharing_subs_people_referred_container.style.width = el.getAttribute("data-stats-width-box")+'px';
                }
                ...
                if(el.getAttribute("data-stats-width-box") != ""){
                sharing_subs_people_position_container.style.width = el.getAttribute("data-stats-width-box")+'px';
                }
                if(el.getAttribute("data-stats-height-box") != ""){
                ...
                if(el.getAttribute("data-stats-width-box") != ""){
                sharing_subs_commission_paid_container.style.width = el.getAttribute("data-stats-width-box")+'px';
                }
                if(el.getAttribute("data-stats-height-box") != ""){
                ...
                stats_commission_icon.innerHTML = `<svg width="40" height="40" viewBox="0 0 40 40" color="${icon_svg_color}" fill="none" xmlns="http://www.w3.org/2000/svg"><g clip-path="url(#clip0_1973_55674)"><rect width="40" height="40" rx="20" fill="${com_color}"/><rect width="40" height="40" fill="${com_color}"/><path d="M14.67 21H12C10.9 21 10 21.9 10 23V28C10 28.55 10.45 29 11 29H14.67C15.22 29 15.67 28.55 15.67 28V22C15.67 21.45 15.22 21 14.67 21Z" fill="currentColor"/><path d="M21.3302 17H18.6602C17.5602 17 16.6602 17.9 16.6602 19V28C16.6602 28.55 17.1102 29 17.6602 29H22.3302C22.8802 29 23.3302 28.55 23.3302 28V19C23.3302 17.9 22.4402 17 21.3302 17Z" fill="currentColor"/><path d="M28.0001 24H25.3301C24.7801 24 24.3301 24.45 24.3301 25V28C24.3301 28.55 24.7801 29 25.3301 29H29.0001C29.5501 29 30.0001 28.55 30.0001 28V26C30.0001 24.9 29.1001 24 28.0001 24Z" fill="currentColor"/><path d="M23.0095 11.8505C23.3195 11.5405 23.4395 11.1705 23.3395 10.8505C23.2395 10.5305 22.9295 10.3005 22.4895 10.2305L21.5295 10.0705C21.4895 10.0705 21.3995 10.0005 21.3795 9.96047L20.8495 8.90047C20.4495 8.09047 19.5395 8.09047 19.1395 8.90047L18.6095 9.96047C18.5995 10.0005 18.5095 10.0705 18.4695 10.0705L17.5095 10.2305C17.0695 10.3005 16.7695 10.5305 16.6595 10.8505C16.5595 11.1705 16.6795 11.5405 16.9895 11.8505L17.7295 12.6005C17.7695 12.6305 17.7995 12.7505 17.7895 12.7905L17.5795 13.7105C17.4195 14.4005 17.6795 14.7105 17.8495 14.8305C18.0195 14.9505 18.3895 15.1105 18.9995 14.7505L19.8995 14.2205C19.9395 14.1905 20.0695 14.1905 20.1095 14.2205L20.9995 14.7505C21.2795 14.9205 21.5095 14.9705 21.6895 14.9705C21.8995 14.9705 22.0495 14.8905 22.1395 14.8305C22.3095 14.7105 22.5695 14.4005 22.4095 13.7105L22.1995 12.7905C22.1895 12.7405 22.2195 12.6305 22.2595 12.6005L23.0095 11.8505Z" fill="currentColor"/></g><defs><clipPath id="clip0_1973_55674"><rect width="40" height="40" rx="20" fill="white"/></clipPath></defs></svg>`;
                ...
                socials.forEach(function(social) {
                    ...
                    the_social_message.style.lineHeight = el.getAttribute("data-social-line-height-name")+'px' || '20px';
                ...
                if((config.settings.horizontal_banner.show_for === "all" && !g.tools.referrer_cookie_present() && referrer_campaign == null) || (config.settings.horizontal_banner.show_for === "referred" && g.tools.referrer_cookie_present() && referrer_campaign != null && referrer_campaign == config.uuid)){
                    ...
                    var position = 'top'
                    var designer = RH.optin_data ? config.settings.horizontal_banner.confirmation : config.settings.horizontal_banner.signup
                    if (!isEmpty(designer)){
                        ...
                        for( var i in designer_data){
                            ...
                            if(el.classList.contains('mtr-text-field') && RH.referrer_name){
                                try{
                                    var substituted_data = el.firstChild.innerHTML.replace(/%referrer_name%/gi, RH.referrer_name);
                                    el.firstChild.innerHTML = g.tools.substitute_phone_number(substituted_data);
                                }
                                catch(h){
                                }
                            ...
                            if (el.id == "template-form"){
                                ...
                                if(RH.optin_data && !config.settings.horizontal_banner.hb_form_present){
                                    hb.style.display = "none";
                                }
                            }else{
                                if(el.classList.contains('mtr-coupon-field')){
                       ...
                       // cl.innerHTML = '<i style="font-family:Material Icons; font-size:25px;" class="material-icons">close</i>';
                ...
                if (c){
                    c.addEventListener("submit", function (J) {
                        ...
                        if (otpCode.length === 6) {
                            ...
                            g.form.codeVerification(data, config, RH);
                        }
                        },100)
            ...
            verificationForm: function(el, config, RH, verification_type, is_signup){
                ...
                mainEle.style.fontFamily = el.getAttribute('data-form-font-family');
                ...
                form_number_input.style.color = "rgb(92, 92, 92)"
                form_number_input.style.backgroundColor = "rgb(244, 244, 249)"
                form_number_input.style.height = "55px"
                form_number_input.style.borderColor = "rgb(255, 255, 255)"
                form_number_input.style.borderWidth = "0px"
                form_number_input.style.borderRadius = "12px"
                form_number_input.style.fontFamily = "inherit"
                form_number_input.style.fontSize = "14px"
                form_number_input.style.width = "100%"
                form_number_input.style.marginBottom = "16px"
                form_number_input.style.textAlign = "center";
                ...
                otpInput.style.color = "rgb(92, 92, 92)";
                otpInput.style.backgroundColor = "rgb(244, 244, 249)";
                otpInput.style.height = "55px";
                otpInput.style.width = "100%";
                otpInput.style.borderColor = "rgb(255, 255, 255)";
                otpInput.style.borderWidth = "0px";
                otpInput.style.borderRadius = "12px";
                otpInput.style.fontFamily = "inherit";
                ...
                if(is_signup){
                    form_submit_button.style.backgroundColor = config.settings.signup_widget_form.submit_button.color;
                    form_submit_button.style.color = config.settings.signup_widget_form.submit_button.label_color;
                    form_submit_button.style.width = '100%';
                    form_submit_button.style.margin = '20px auto';
                    form_submit_button.style.borderWidth = "0px"
                    form_submit_button.style.cursor ='pointer';
                    form_submit_button.style.borderRadius = config.settings.signup_widget_form.submit_button.corner_roundness;
                    form_submit_button.style.fontSize = config.settings.signup_widget_form.submit_button.font_size;
                    form_submit_button.style.height = config.settings.signup_widget_form.submit_button.height;
                    form_submit_button.style.fontWeight = config.settings.signup_widget_form.submit_button.bold ? 'bolder' : 'normal';
                    form_submit_button.style.textDecoration = config.settings.signup_widget_form.submit_button.underline ? 'underline' : 'none';
                    form_submit_button.style.fontStyle = config.settings.signup_widget_form.submit_button.italic ? 'italic' : 'none';
                    form_submit_button.style.fontFamily = g.tools.findFont(config.settings.signup_widget_form.submit_button.font_family);
                }else{
                    form_submit_button.style.backgroundColor = config.settings.form.submit_button.color;
                    form_submit_button.style.color = config.settings.form.submit_button.label_color;
                    form_submit_button.style.width = '100%';
                    form_submit_button.style.margin = '20px auto';
                    form_submit_button.style.borderWidth = "0px"
                    form_submit_button.style.cursor ='pointer';
                    form_submit_button.style.borderRadius = config.settings.form.submit_button.corner_roundness;
                    form_submit_button.style.fontSize = config.settings.form.submit_button.font_size;
                    form_submit_button.style.height = config.settings.form.submit_button.height;
                    form_submit_button.style.fontWeight = config.settings.form.submit_button.bold ? 'bolder' : 'normal';
                    form_submit_button.style.textDecoration = config.settings.form.submit_button.underline ? 'underline' : 'none';
                    form_submit_button.style.fontStyle = config.settings.form.submit_button.italic ? 'italic' : 'none';
                    form_submit_button.style.fontFamily = g.tools.findFont(config.settings.form.submit_button.font_family);
                }
                ...
                form_resend_button.style.fontFamily = g.tools.findFont(config.settings.form.submit_button.font_family);
                form_resend_button.style.fontSize = config.settings.form.submit_button.login_font_size;
                form_resend_button.style.color = config.settings.form.submit_button.login_text_colour || '#000000';
            ...
            quickadd: function (config, RH) {
                ...
                if (config.settings.quick_add_form.terms_conditions.require) {
                    ...
                    A.id = "mtr-form-tc-text";
                    A.setAttribute("for", "mtr-form-tc-checkbox");
                    A.innerText = config.settings.quick_add_form.terms_conditions.text;
                ...
                p.addEventListener("click", function (J) {
                    ...
                    J.preventDefault();
                    g.tools.getSessionCookie(!0, config, RH);
                ...
                if (config.settings.signup_widget_form.terms_conditions.require) {
                    ...
                    A.id = "mtr-form-tc-text";
                    A.setAttribute("for", "mtr-form-tc-checkbox");
                    A.innerText = config.settings.signup_widget_form.terms_conditions.text;
                    ...
                    var z = mtg("a");
                    z.id = "mtr-form-tc-link";
                    z.setAttribute("href", config.settings.signup_widget_form.terms_conditions.url);
                    z.setAttribute("target", "_blank");
                    z.innerText = "\ud83d\udd17";
                    A.appendChild(z);
                    v.appendChild(x);
                ...
                config.settings.signup_widget_form.terms_conditions.require && d.appendChild(v);
                d.appendChild(y);
                d.appendChild(st);
                t = mtg("div");
                ...
                c.addEventListener("submit", function (J) {
                    ...
                    "valid" == form_validity ? config.settings.recaptcha.enable && C ? grecaptcha.execute(F) : (data = g.tools.getFormValues(c, RH), g.form.submit(data, config, RH, 0, config.settings.signup_widget_form, "signup_widget")) : ("form_incomplete" == form_validity ? alert_or_console(config.settings.alerts.form_incomplete) : "terms_not_accepted" == form_validity ? alert_or_console(config.settings.alerts.terms_conditions) : "invalid_phone_number" == form_validity &&
                        alert_or_console(config.settings.alerts.invalid_phone_number), p.disabled = !1, p.innerText = q ? config.settings.signup_widget_form.submit_button.check_position : config.settings.signup_widget_form.submit_button.text)
                });
                (config.settings.design.powered_by || config.settings.design.powered_by) && c.appendChild(g.generate.poweredBy("form", config));
                config.settings.recaptcha.enable && "" != config.settings.recaptcha.public_key.trim() && g.libraries.recaptcha(config, RH);
                a.appendChild(c);
            ...
            }, thankyou_screen: function (config) {
                var th = mtg("div");
                if (!isEmpty(config.settings.signup_widget_form.thankyou_screen_settings)){
                        ...
                        th.innerHTML = "";
                        th.style.maxWidth = "95%";
                        th.style.borderRadius = t_el.style.borderRadius;
                        th.appendChild(t_el);
                    }else{
                        th.innerHTML =`<div id="thankyou-widget" style="width:550px; max-width: 95%; margin-top: 150px;">
                            <div id="thankyou-form" class="mt-30" data-bg="color" data-bg-size="auto" data-bg-color="#ffffff" style="border-style: solid none none none; border-width: 4px;">
                                <div class="mtr-text-field drag design-field-column first-el" style="border-top-left-radius: 23px; border-top-right-radius: 23px;" id="text-div-0001">
                                    <p id="text-input-0001" style="padding: 0px; font-weight: 500; font-size: 30px; color: rgb(43, 47, 62); overflow-wrap: break-word; width: 90%; text-align: center;">Thank you for signing up </p>
                                </div>
                            </div>
                        </div>`;
                    }
                ...
                main_div.classList.add("mtr-overflow-hidden", "mtr-border", "mtr-ring-opacity-5", "sm:mtr-rounded-lg", "mtr-lb-div")
                ...
                //return "<table id='mtr-lb-table' cellpadding='0' cellspacing='0'>" + a + "<tbody>" + c + "</tbody></table>"
                ...
                c += `<tr><td colspan="100%" style="text-align: center;"><div style="width: 100%; display: flex; align-items: center; justify-content: center;">`;
                ...
                paginatedData.forEach(function(k, h) {
                    ...
                    sortedPositions?.forEach((columnName) => {
                        ...
                        switch (columnName) {
                            ...
                            break;
                            case "value":
                            dataValue = parseFloat(k.value).toFixed(2);
                            break;
                        }
                        var text_align = el.getAttribute('data-align-table');
                        var font_size = el.getAttribute('data-referrals-table-font-size')+'px';
        ...
        setTimeout(function(){
            for (var i = 0; i < campaign_uuids.length; i++) {
                ...
                g.generate.stylesheet("#mtr-form-fields input::-webkit-input-placeholder {color: "+pc+";}");
                ...
                if( config[uuid].settings.sharing.verification.qr_code_enabled && !global_config["enableQR"] ){
					global_config["enableQR"] = true
                    g.libraries.qr();
                }
                ...
                window[rh_variable].form = {
                    submit: function (a) {
                        g.form.submit(a, config[uuid], window[rh_variable]);
                ...
                var N = function () {
                        if (Object.keys(r).every(function (c) {
                            ...
                            if(g.tools.getParams("mtrVerifyInstall") == config[uuid].uuid){
                                ...
                                g.tools.ajax.request(g.tools.getWidgetUrl(config) + "/update_next_steps", "POST", { id: "installation_link" }, function(response) {
                                }, function() {
